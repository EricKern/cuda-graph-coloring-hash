 ...
 */
void simple_tiling(const int m_rows, const int number_of_tiles, const int *const row_ptr, const int *const col_ptr, int ** const slices_dptr, int ** const ndc_dptr, int ** const offsets_dptr)
{
	int indices_[2]={0,m_rows};
	int number_of_slices=1, *slices_=new int[m_rows], *layers_=new int[m_rows];
	...
	*slices_dptr=slices_;
	tiling_partitioning(m_rows,number_of_tiles,number_of_slices,row_ptr,col_ptr,layers_,indices_,slices_,ndc_dptr,slices_dptr,NULL,offsets_dptr,NULL,&number_of_slices,0);
	delete [] layers_;
}
...
{
	int * row_ptr;	int * col_ptr;	double * val_ptr;					// create pointers for matrix in csr format
	const int m_rows=cpumultiplyDloadMTX( inputMat,&row_ptr,&col_ptr,&val_ptr);
	double * mult_ptr=new double[m_rows];	double * sol_ptr=new double[m_rows];	double * prm_ptr=new double[m_rows];
	...
	for(int i=0; i<m_rows; i++)
		mult_ptr[i]=i*0.1;
	simple_tiling(m_rows, number_of_tiles, row_ptr, col_ptr, &slices_, &ndc_, &offsets_);
	// spMV without permutation
	cpumultiplyDspmv(number_of_tiles, row_ptr, col_ptr, val_ptr, mult_ptr, ndc_, sol_ptr);
	// spMV with previous permutation and successive unpermutation
	cpumultiplyDpermuteMatrix(number_of_tiles, 1, ndc_, slices_, row_ptr, col_ptr, val_ptr, &row_ptr, &col_ptr, &val_ptr, true);
	cpumultiplyDpermuteVector(number_of_tiles, 1, ndc_, slices_, mult_ptr, &mult_ptr, true);
	cpumultiplyDspmv(number_of_tiles, row_ptr, col_ptr, val_ptr, mult_ptr, ndc_, prm_ptr);
	cpumultiplyDpermuteVector(number_of_tiles, 1, ndc_, slices_, prm_ptr, &prm_ptr, false);
...
{
	int * row_ptr;	int * col_ptr;	float * val_ptr;					// create pointers for matrix in csr format
	const int m_rows=cpumultiplySloadMTX( inputMat,&row_ptr,&col_ptr,&val_ptr);
	float * mult_ptr=new float[m_rows];	float * sol_ptr=new float[m_rows];	float * prm_ptr=new float[m_rows];
	int * ndc_;															// array with indices of each tile in all slices
	int * slices_;														// array with nodes grouped in slices
	int * offsets_;
	// initialise reference array
	...
	simple_tiling(m_rows, number_of_tiles, row_ptr, col_ptr, &slices_, &ndc_, &offsets_);
	// spMV without permutation
	cpumultiplySspmv(number_of_tiles, row_ptr, col_ptr, val_ptr, mult_ptr, ndc_, sol_ptr);
	// spMV with previous permutation and successive unpermutation
	cpumultiplySpermuteMatrix(number_of_tiles, 1, ndc_, slices_, row_ptr, col_ptr, val_ptr, &row_ptr, &col_ptr, &val_ptr, true);
	cpumultiplySpermuteVector(number_of_tiles, 1, ndc_, slices_, mult_ptr, &mult_ptr, true);
	cpumultiplySspmv(number_of_tiles, row_ptr, col_ptr, val_ptr, mult_ptr, ndc_, prm_ptr);
	cpumultiplySpermuteVector(number_of_tiles, 1, ndc_, slices_, prm_ptr, &prm_ptr, false);
	// compare of results with and without permutation 
...
int main(int argc, char* argv[])
{
	...
	return (argc==3) ? float_multiply( inputMat, number_of_tiles) : float_multiply( inputMat, number_of_tiles) ; 
}
