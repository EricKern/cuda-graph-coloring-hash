...
int load_mtx(const char * const inputMat, int ** const row_dptr, int ** const col_dptr, prcsn ** const val_dptr)
{
	assert(NULL!=inputMat);												// path to the file with matrix information(<name>.mtx) or matrix generation dimensions(XxYxZ) must be set
	...
	bool m_sizes = true, m_header=true, m_symm=true;					// flag which signals if matrix interiors were found in the file
	int row_sz=-1, col_sz=-1, val_sz=-1;								// initial values for matrix interior variables
	...
	if (!ifs_.is_open())												// open stream
		throw std::runtime_error("cannot open file '" + file_ + "' for reading");
	while(std::getline(ifs_, line_))									// read in content of file line by line
	{
		...
		if ( sstr.peek() =='%' ) 										// if the first character is percentage sign, it is comment line and should be omitted
		{
			if(m_header)
			{
				m_header=false;
		...
		{
			sstr >> row_sz >> col_sz >> val_sz;							// read in the information about matrix interiors
			if(row_sz > 0 && col_sz > 0 && val_sz > 0) 					// only if all of them have plausible values
			{
				m_sizes = false;										// set the flag that the matrix information was read in
				col_inter.resize(row_sz); val_inter.resize(row_sz);		// resize the vectors appropriately
			}
			else
			{
				ifs_.close();											// close input file stream
				throw std::runtime_error("invalid dimesions of matrix from file '" + file_ + "'");
			}
		}
		else															// non zero entry of matrix is described by three values => row, column and value
		{
			int row=-1, col=-1;											// initialize indices with values which allow later check up
			prcsn val;
			sstr>> row >> col >> val;									// read in the next matrix entry from the stream
			
			if((row--)>0 && (col--)>0 && row<row_sz && col<col_sz)		// only if both indices have plausible values convert them to zero based indices
			{
	...
	for(size_t idx_=0; idx_<row_sz; idx_++)								// go over the rows
	{
		...
		std::copy ( col_inter.at(idx_).begin(), col_inter.at(idx_).end(), out_col_ptr+row_ind );	// copy the entries of column indices of that row
		std::copy ( val_inter.at(idx_).begin(), val_inter.at(idx_).end(), out_val_ptr+row_ind );	// copy the entries of values of that row
		row_ind+=col_inter.at(idx_).size();								// increment row index by the number of written non zero entries
	}
	out_row_ptr[row_sz]=row_ind;										// last entry contains the number of all non zero entries in matrix
	if(NULL==row_dptr)
		delete out_row_ptr;												// delete created row data
	else
		*row_dptr=out_row_ptr;											// redirect output row pointer to output memory location
	if(NULL==col_dptr)
		delete out_col_ptr;												// delete created column data
	else
		*col_dptr=out_col_ptr;											// redirect output column pointer to output memory location
	if(NULL==val_dptr)
		delete out_val_ptr;												// delete created value data
	else
		*val_dptr=out_val_ptr;											// redirect output value pointer to output memory location
	return row_sz;
}
 ...
 * @return				: number of rows in read-in matrix
 */						
int cpumultiplyDloadMTX(const char * const inputMat, int ** const row_dptr, int ** const col_dptr, double ** const val_dptr)
{
 ...
 * @return				: number of rows in read-in matrix
 */						
int cpumultiplySloadMTX(const char * const inputMat, int ** const row_dptr, int ** const col_dptr, float ** const val_dptr)
{
