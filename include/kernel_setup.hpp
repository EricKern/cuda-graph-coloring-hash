...
void kernel_setup(const char* inputMat,
  ...
  static constexpr int max_shmem = 19 * 1024;
  static constexpr int nedded_shmem = distance2 ? max_shmem / 2 : max_shmem;
  static constexpr int shmem_lim = std::max(nedded_shmem, (int)sizeof(TempStorageT));

  std::cout << "Max shmem usage per block: " << shmem_lim << std::endl;
