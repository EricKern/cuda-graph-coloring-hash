  ...
  for(int blk_i = blockIdx.x; blk_i < tiles; blk_i += gridDim.x) {
    ...
    for (int k = 0; k < num_hashes; ++k) {
      ...
      for (int i = 0; i < num_bit_widths; ++i) {
        ...
        if(threadIdx.x == 0){
          coop_total1[k].m[i] = cub::Sum{}(l1_sum, coop_total1[k].m[i]);
          coop_max1[k].m[i] = cub::Max{}(l1_max, coop_max1[k].m[i]);
        }
      }
    }
  }
    ...
    for (int k = 0; k < num_hashes; ++k) {
      ...
      for (int i = 0; i < num_bit_widths; ++i) {
        ...
        if(threadIdx.x == 0){
          ...
          coop_total2[k].m[i] = cub::Sum{}(l1_sum2, coop_total2[k].m[i]);
          coop_max2[k].m[i] = cub::Max{}(l1_max2, coop_max2[k].m[i]);
        }
      } // bit_w
    } // hash_nr
  } // blk_i
