 ...
 */
char* stage_handout(const int tn,
                    const int stage,
                    const int* const prt_,
                    char* reports_) {
	if (NULL ==
	    reports_) // if provided pointer is NULL output on stdout is desired
	{
	...
	{
		reports_ +=
		    sprintf(reports_, "      stage %d: %d %d", stage, prt_[0], prt_[1]);
		for (int idx = 2; idx < tn; idx++)
 ...
 */
void form_tiles(const int s_id,
                const int tn,
                const int slice_start,
                const int* const tiles_,
                const int* const slices_,
                const char* const reports_,
                int* const prt_,
                char** const stats_dptr,
                int* const ndc_,
                int* const write_ptr,
                const bool s_info) {
	int* const __restrict__ cp_v =
	    new int[tn + 1]; // array to hold the copies of tile indices
	int slice_end =
	    slice_start; // index of the first entry of the slice within the array slices
	for (int n_idx = 0, var_idx = s_id * tn; n_idx < tn;
	     n_idx++) // go over the tiles
	{
		...
		slice_end += prt_
		    [n_idx]; // read in the number of nodes in the next tile and increment the index accordingly
	}
	cp_v[tn] =
	    slice_end; // store last index which marks the beginning of following slice

	if (NULL !=
	    write_ptr) // if pointer for output writing is NULL no output is desired
	{
		int* const __restrict__ hlp_v = new int
		    [slice_end -
		     slice_start]; // array to hold the copies of node identification numbers
#pragma omp parallel num_threads(tn)
		{
			const int t_id =
			    omp_get_thread_num(); // each thread register nodes from one tile
			const int* rd_ptr = slices_ + slice_start - 1; // set read pointer
			int* wrt_ptr =
			    hlp_v + (cp_v[t_id] - slice_start); // set write pointer
			const int* const stp_ptr =
			    hlp_v + (cp_v[t_id + 1] - slice_start); // set stop pointer
			while (
			    wrt_ptr <
			    stp_ptr) // as long as write pointer is in front of stop pointer
			{
				...
				if (t_id !=
				    tiles_
				        [node_]) // if node is not from the same tile as thread number
					continue; // go to the next entry in slices_
				*(wrt_ptr++) =
				    *rd_ptr; // overwrite the node identification numbers in slices with sorted ones from hlp_v array
			}
	...
	if (NULL != reports_ || s_info) // if tinf or sinf flags were set
	{
		if (NULL == *stats_dptr) // if stdout should be used for printing
		{
			...
			if (s_info) // if optional argument in order to print messages about layers is set to true
			{
				...
				for (int prt_idx = 1; prt_idx < tn;
				     prt_idx++) // go over the tiles
				{
					const int prt_val = prt_[prt_idx];
					if (prt_val >
					    prt_max) // if tile size is bigger than the maximal one
						prt_max = prt_val; // overwrite the maximal value
					if (prt_val <
					    prt_min) // if tile size is bigger than the maximal one
		...
		{
			*stats_dptr +=
			    sprintf(*stats_dptr, "statistic for slice %d:\n", s_id);
			if (s_info) // if optional argument in order to print messages about layers is set to true
			{
				const int prt_init = prt_[0];
				int prt_min = prt_init;
