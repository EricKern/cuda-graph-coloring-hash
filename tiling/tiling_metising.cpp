 ...
 */
inline void metis_call(idx_t tn,
                       idx_t slice_sz,
                       const int u_factor,
                       idx_t* xadj,
                       idx_t* adjncy,
                       idx_t* adjwgt,
                       idx_t* const __restrict__ part_) {
	idx_t one_ = 1; //
	idx_t
	...
	options[METIS_OPTION_UFACTOR] =
	    u_factor; // Specifies the maximum allowed load imbalance among the partitions. A value of x indicates that the allowed load imbalance is (1 + x)/1000. For -ptype=rb, the default value is 1 (i.e., load imbalance of 1.001) and for -ptype=kway, the default value is 30 (i.e., load imbalance of 1.03).
	options[METIS_OPTION_NCUTS] =
	    3; // Specifies the number of different partitionings that it will compute. The final partitioning is the one that achieves the best edgecut or communication volume. Default is 1.
	options[METIS_OPTION_NUMBERING] =
	    0; // Used to indicate which numbering scheme is used for the adjacency structure of a graph or the element-node structure of a mesh.
 ...
 */
inline void metis_tiles(const int tn,
                        const int slice_sz,
                        const int* const __restrict__ slices_,
                        const idx_t* const __restrict__ part_,
...
#pragma omp parallel num_threads(tn)
	{
		...
		while (++rd_ptr <
		       stp_ptr) // as long as write pointer is in front of stop pointer
			if (t_id == *rd_ptr) // if thread id equals tile number
			{
				tiles_[slices_[rd_ptr - part_]] =
...
#define WEIGHT_UNASSIGNED \
	2 // one of by the edge connected nodes is still unassigned to a tile
#define WEIGHT_ASSIGNED_SAME \
	2 // both of by the edge connected nodes are assigned to the same tile
#define WEIGHT_ASSIGNED_DIFFERENT \
	1 // both of by the edge connected nodes are assigned to different tiles
 ...
 * @param[out] part_		: array with partition information
 */
inline void metising(const int s_id,
                     const int tn,
                     ...
                     const int* const __restrict__ slice_spot,
...
#pragma omp parallel num_threads( \
    tn) // check the first condition (maximal connections for a tile in neighbouring layer)
	{
		const int t_id = omp_get_thread_num(); // thread own gang id
		int* const __restrict__ scn_ = new int
		    [tn]; // each thread allocates memory space for counting the number of connections to the nodes in tiles in the neighbouring slice
		const int end_border =
		    borders_[t_id + 1] +
		    slice_start; // end of area for processing of current threads
		for (int n_idx = slice_start + borders_[t_id]; n_idx < end_border;
		     n_idx++) // go over the entries
		{
			std::memset(
			    scn_, 0, sizeof(int) * tn); // initialize scn array with zeroes
			const int node_ = slices_[n_idx]; // node number
			const int row_end =
			    row_ptr[node_ + 1]; // index where following row starts
			for (int row = row_ptr[node_]; row < row_end;
			     row++) // go over row entries
			{
				const int nei_ =
				    col_ptr[row]; // read in the node number of the neighbor
				if (s_id - layers_[nei_] ==
				    cond_) // if a node is in the neighboring layer
					scn_[tiles_
					         [nei_]]++; // register for a tile its connected to
			}
			int part_idx =
			    NO_TILE_ASSIGNMENT; // index to a tile if specific node is connected only with nodes from this tile
			for (int vl = 0; vl < tn;
			     vl++) // go over the gathered information about the connections
				if (0 != scn_[vl]) // if at least one connection is detected
				{
					if (NO_TILE_ASSIGNMENT !=
					    part_idx) // if the index is set, then connections to multiple tiles are detected
					{
			...
			if (part_idx >
			    NO_TILE_ASSIGNMENT) // if node is connected only to nodes from one tile
				tiles_[node_] =
				    part_idx; // register the node for a particular tile
		}
		delete[] scn_;
	}
	...
	std::vector<idx_t>
	    adjwgt; // The weights of the edges (if any) are stored in an additional array called adjwgt which contains 2m elements
	     ...
	     n_idx++) // go over the nodes in the largest slice
	{
		...
		for (int row = row_ptr[node_]; row < row_end; row++) // go over row
		{
			...
			{
				adjncy.push_back(
				    slice_spot[nei_]); // store its index in the adjncy vector
				if (tiles_[node_] < 0 ||
				    tiles_[nei_] <
				        0) // if either the node or its neighbor is not assigned to a tile yet
					adjwgt.push_back(
					    WEIGHT_UNASSIGNED); // the weight of such an edge is small
				else {
					if (tiles_[node_] !=
					    tiles_
					        [nei_]) // if the nodes don't belong to the same tile
						adjwgt.push_back(
						    WEIGHT_ASSIGNED_DIFFERENT); // the weight of such an edge is medium
					else
						adjwgt.push_back(
						    WEIGHT_ASSIGNED_SAME); // the weight of an edge between two nodes of the same tile is large
				}
			}
		}
		xadj[++m_idx] =
	...
	metis_call(
	    tn, slice_sz, 111, xadj.data(), adjncy.data(), adjwgt.data(), part_);
