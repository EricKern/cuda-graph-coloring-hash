...
inline void get_borders(const int m_rows, const int tn, int * const borders_)
{
	...
	const int up_worker=m_rows%tn;										// amount of threads that handle one more element as minimal work area
	...
	#pragma omp parallel num_threads(tn)
	{
		...
		if(t_id<=up_worker)												// if thread belongs to those with maximal workload
			borders_[t_id]=t_id*work_max;								// write out the begin of workload area in matrix for each thread
		else															// if thread belongs to those with minimal workload
			borders_[t_id]=up_worker*work_max+(t_id-up_worker)*work_min;// write out the begin of workload area in matrix for each thread
		if(tn-1==t_id)													// the last thread in the gang
			borders_[t_id+1]=m_rows;									// write out the end of workload area in matrix for the last thread
	}
}
...
double tiling_equipartitioning(const int m_rows, const int tn, int ** ndc_dptr)
{
	...
	get_borders(m_rows, tn, ndc_ptr);									// division of workload among participating threads
	end_ = Clock::now();												// stop of the time measurement
	*ndc_dptr=ndc_ptr;													// redirect output pointer to output memory location
	return DSeconds(end_ - start_).count();								// return elapsed time spend for equipartitioning
}
