 ...
 */
int * offset_multi( const int tn, const int n_z, const int ndc_max, const int * const row_ptr, const int * const col_ptr, const int * const ndc_, const int * const slices_, const int * const tiles_, int * const write_ptr )
{
	int * const __restrict__ offsets_=new int[tn*n_z];
	#pragma omp parallel num_threads(tn)
	{
		int * const __restrict__ cpy_arr =new int[ndc_max];				// each thread allocates memory space for copying
		...
		for(int t_idx=n_z*t_id, current_tile, next_tile=ndc_[t_idx];t_idx<t_end;t_idx++)	// go over the layers
		{
			...
			int cpy_begin=-1, cpy_end=next_tile-current_tile;			// indices for writing into copy array (redundant in front)
			for(int ndc_idx=current_tile;ndc_idx<next_tile;ndc_idx++)	// go over the entries of a tile in one layer
			{
				...
				for(int row=row_ptr[node_]; row<row_end; row++)			// go over row entries
				{
					...
					if(node_tile!=tiles_[nei_])							// if the neighbor and the node have different tile numbers
					{
						covered=false;									// signal that the node has redundant entry
						cpy_arr[++cpy_begin]=node_;						// write node in front part of copy array
						break;
					}
				}
				if(covered)												// if no redundant entries were found
					cpy_arr[--cpy_end]=node_;							// write node in back part of copy array
			}
			offsets_[t_idx]=cpy_end+current_tile;						// write out index marking line between nodes with and without redundant values for current tile
			for(int ndc_idx=current_tile;ndc_idx<next_tile;ndc_idx++)	// go over the entries of a tile in one layer
				write_ptr[ndc_idx]=cpy_arr[ndc_idx-current_tile];		// write ordered tile into array slices_
		}
