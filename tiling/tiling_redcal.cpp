 ...
 */
void redcal_multi(const int m_rows,
                  const int tn,
                  const int n_z,
                  ...
                  char* stats_ptr) {
	int* const __restrict__ redundant_nodes =
	    new int[tn]; // stores the number of non-redundant nodes for each bar
	int* const __restrict__ outside_edges =
	    new int[tn]; // stores the number of redundant connections for each bar
	int* const __restrict__ overall_edges =
	    new int[tn]; // stores the number of overall connections for each bar
#pragma omp parallel num_threads(tn)
	{
		int inside_edges_thread = 0, outside_edges_thread = 0,
		    redundant_nodes_thread =
		        0; // threadwise variables to collect non-redundant and redundant connections for each bar
		     ...
		     t_idx++) // go over the layers
		{
			     ...
			     ndc_idx++) // go over the entries of a tile in one layer
			{
				...
				const int node_tile =
				    tiles_[node_]; // read in the tile number the node is in
				const int row_end =
				     ...
				     row++) // go over row entries
				{
					...
					if (node_tile ==
					    tiles_
					        [nei_]) // if the entry and the node have the same tile number
		...
		overall_edges[t_id] =
		    inside_edges_thread +
		    outside_edges_thread; // total number of edges in a bar
		outside_edges[t_id] =
		    outside_edges_thread; // number of redundant connections
		redundant_nodes[t_id] =
		    redundant_nodes_thread; // number of non-redundant nodes
	} // end #pragma omp parallel
	double sum = 0.0, min = 1, max = 0, prc, redundant_nodes_sum = 0.0;
	for (int bar_ = 0; bar_ < tn; bar_++) // go through bars
	{
		prc =
		    (0 == overall_edges[bar_])
		        ? 0.0
		        : ((double) outside_edges[bar_]) /
		              overall_edges
		                  [bar_]; // calculated relative number of redundant connections for a bar
		if (prc < min)
			min = prc;
		if (prc > max)
			max = prc;
		sum +=
		    prc; // sum up relative numbers of redundant connections of all bars in domain
		redundant_nodes_sum += redundant_nodes
		    [bar_]; // sum up non-redundant nodes of all bars in domain
	}
	delete[] outside_edges;
	delete[] overall_edges;
	delete[] redundant_nodes;
	if (NULL == stats_ptr) // if stdout should be used for printing
	{
		printf(
		    "Statistics for redundant calculations:\n"
		    "   Redundant edges:\n"
		    ...
		    min * 100,
		    max * 100,
		    redundant_nodes_sum * 100 / m_rows);
	}
